...
- Gemini routing budget: treat `gemini-3-flash-preview` as capped at `20` peak requests/day and reserve it for learning and summary tasks; treat `gemini-3.1-flash-lite` as capped at `500` peak requests/day and use it first for organizing, enrichment, relation judging, and routine ask flows. Treat `gemma-4-31b-it` as the richer Google-hosted fallback after Flash Lite, with a default cap of `100` peak requests/day. Env overrides are `GEMINI_DAILY_REQUEST_LIMIT`, `GEMINI_FLASH_LITE_DAILY_REQUEST_LIMIT`, and `GEMMA_31B_DAILY_REQUEST_LIMIT`.
