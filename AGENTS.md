...
- `instance-ops`: Connect to the Ubuntu bot instance, inspect service status and logs, restart one or all bots, and move files between local and remote for safe edits. Use when work requires checking the instance state, reading `journalctl` logs, restarting `groteskbot.service` / `tsekbot.service` / `usefulbot.service` / `svitlobot.service`, or editing, adding, downloading, uploading, or deleting files on the instance. File: `skills/instance-ops/SKILL.md`
