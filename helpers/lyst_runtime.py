...
def get_sale_emoji(sale_percentage, uah_sale):
    # The owner recognizes these exact tiers in Telegram, so keep the visual scheme
    # stable instead of letting later cleanup drift the headline emojis again.
...
def build_shoe_message(shoe, sale_percentage, uah_sale, kurs, kurs_symbol, old_sale_price=None, status=None):
    ...
    # These labels intentionally match the operator's preferred chat format exactly.
