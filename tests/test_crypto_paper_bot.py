    ...
    frame = pd.DataFrame(
        {
            ...
            "bb_bandwidth_pct_rank_100": [0.5] * rows,
            "volume_sma_20": [100.0] * rows,
            "ema_20": [102.0] * rows,
            "ema_50": [105.0] * rows,
            "ema_200": [95.0] * rows,
            "ema_50_slope_5": [0.02] * rows,
            "rsi_14": [45.0] * rows,
            "adx_14": [25.0] * rows,
            "avg_dollar_volume_24": [3_000_000.0] * rows,
            "macd_line": [-0.5] * rows,
            "macd_signal": [-0.4] * rows,
            "macd_hist_slope": [0.1] * rows,
        }
    )
    return frame
...
class StrategyEngineTests(unittest.TestCase):
    ...
    def test_algo_4_improved_squeeze_breakout_true(self) -> None:
        frame = make_indicator_frame()
        frame.loc[219, "bb_bandwidth_pct_rank_100"] = 0.1
        frame.loc[219, "close"] = 120.0
        frame.loc[219, "bb_upper"] = 110.0
        frame.loc[218, "high"] = 118.0
        frame.loc[219, "volume"] = 170.0
        frame.loc[219, "volume_sma_20"] = 100.0
        self.assertTrue(cpb.StrategyEngine.algo_4_improved_squeeze_breakout(frame))

    def test_algo_6_improved_reversal_requires_range_and_liquidity(self) -> None:
        frame = make_indicator_frame()
        frame.loc[:, "adx_14"] = 10.0
        frame.loc[218, "macd_line"] = -0.7
        frame.loc[218, "macd_signal"] = -0.6
        frame.loc[219, "macd_line"] = -0.2
        frame.loc[219, "macd_signal"] = -0.3
        frame.loc[218, "rsi_14"] = 49.0
        frame.loc[219, "rsi_14"] = 51.0
        frame.loc[218, "high"] = 100.0
        frame.loc[219, "close"] = 101.0
        self.assertTrue(cpb.StrategyEngine.algo_6_improved_reversal(frame))
    ...
    def test_evaluate_all_false_when_history_is_short(self) -> None:
        ...
        self.assertEqual(
            ...
            {algorithm: False for algorithm in cpb.ALGORITHM_ORDER},
        )
...
class TradeRepositoryTests(unittest.TestCase):
    ...
    def test_bootstrap_seeds_accounts_once(self) -> None:
        ...
        self.assertEqual(first, {algorithm: 10000.0 for algorithm in cpb.ALGORITHM_ORDER})
...
class BotOrchestrationTests(unittest.TestCase):
    ...
    def test_same_candle_tp_and_sl_resolves_as_loss(self, _sleep: mock.Mock) -> None:
        ...
        bot = cpb.CryptoPaperBot(self.repo, FakeGateway({"BTC/USDT": frame}), notifier)
    ...
    def test_find_new_setups_opens_only_one_trade_per_coin(self, _sleep: mock.Mock) -> None:
        ...
        self.assertEqual(len(notifier.balance_updates), 1)
    ...
    def test_run_cycle_skips_ranked_symbol_that_already_has_open_trade(self, _sleep: mock.Mock) -> None:
        ...
        self.assertGreaterEqual(len(notifier.balance_updates), 1)

    def test_trade_and_balances_messages_are_split(self) -> None:
        ...
        message = cpb.build_close_message(closed_trade)
    ...
    @mock.patch.object(cpb.requests, "post")
    def test_telegram_notifier_creates_and_pins_balances_message(self, post: mock.Mock) -> None:
        response_send = mock.Mock()
        response_send.raise_for_status.return_value = None
        response_send.json.return_value = {"ok": True, "result": {"message_id": 77}}

        response_chat = mock.Mock()
        response_chat.raise_for_status.return_value = None
        response_chat.json.return_value = {"ok": True, "result": {}}

        response_pin = mock.Mock()
        response_pin.raise_for_status.return_value = None
        response_pin.json.return_value = {"ok": True, "result": True}

        post.side_effect = [response_send, response_chat, response_pin]
        ...
        self.assertIn("getChat", post.call_args_list[1].args[0])
        self.assertIn("pinChatMessage", post.call_args_list[2].args[0])
